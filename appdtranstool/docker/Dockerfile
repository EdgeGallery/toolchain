...
ENV APP_FILE appPkgTransTool.jar
...
RUN chmod -R 750 $APP_HOME &&\
    ...
    mkdir -p -m 550 $APP_HOME/configs &&\
...
COPY --chown=$USER_NAME:$GROUP_NAME target/classes/configs /usr/app/configs
